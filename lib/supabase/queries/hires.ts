...
export async function fetchHireApplications(): Promise<HireApplication[]> {
  const [internsRes, specialistsRes] = await Promise.all([
    ...
    supabase
      ...
      .select('id, first_name, last_name, email, phone, message, status, created_at, referer')
      .order('created_at', { ascending: false }),
  ]);
  ...
  const interns: HireApplication[] = (internsRes.data ?? []).map((r: any) => ({ ...r, kind: 'intern' as const }));
  const specialists: HireApplication[] = (specialistsRes.data ?? []).map((r: any) => ({ ...r, kind: 'specialist' as const }));
